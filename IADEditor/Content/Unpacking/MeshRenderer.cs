...
public class MeshRenderer : ViewModelBase
{
    ...
    public Point3D OffsetCameraPosition =>
        new Point3D(CameraPosition.X + CameraTarget.X, CameraPosition.Y + CameraTarget.Y,
    ...
    public MeshRenderer(MeshLOD lod, MeshRenderer old)
    {
        ...
        // Calculate vertex size minus the position and normal vector
        var offset = lod.Meshes[0].VertexSize - 3 * sizeof(float) - sizeof(int) - 2 * sizeof(short);

        // In order to set up the camera position and target properly, I need to figure out how big
        // the object that is rendered is. Hence, I need to know its bounding box.
        ...
        // This is done to unpack the packed normals
        ...
        foreach (var mesh in lod.Meshes)
        {
            ...
            using (var reader = new BinaryReader(new MemoryStream(mesh.Vertices)))
            {
                for (int i = 0; i < mesh.VertexSize; ++i)
                {
                    ...
                    var normalZ =
                        Math.Sqrt(Math.Clamp(1f - (normalX * normalX + normalY * normalY), 0f, 1f)) *
                        ((signs & 0x2) - 1f);
